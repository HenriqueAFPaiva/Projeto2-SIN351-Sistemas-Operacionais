...
//Criando uma struct para uma fila, usada no FIFO para armazenar as páginas
//que serão removidas
...
int fifo(int8_t** page_table, int num_pages, int prev_page, int fifo_frm, int num_frames, int clock) {
  ...
  extern Fila f;
  //int page recebe a primeira página da fila, ou seja, recebe a página que
  //será removida
  int page = f.itens[f.inicio];
  //f.inicio vai para a próxima posição da fila
  f.inicio = ++f.inicio % num_frames;
...
int simulate(int8_t **page_table, int num_pages, int *prev_page, int *fifo_frm, int *physical_memory, int *num_free_frames, int num_frames, int *prev_free,
            int virt_addr, char access_type, eviction_f evict, int clock) {
    ...
    //adiciona a página nova no final da fila
    extern Fila f;

    f.itens[f.fim] = virt_addr;
    //f.fim vai para a próxima posição da fila, para que a próxima página
    //seja inserida na posição correta
    f.fim = ++f.fim % num_frames;
...
int main(int argc, char **argv) {
    ...
    //Alocando e inicializando a Fila criada
    extern Fila f;
    f.itens = (int*) malloc (num_frames * sizeof(int));
    f.inicio = 0;
    f.fim = -1;
